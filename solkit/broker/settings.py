...
class BrokerKafkaSettings(BaseSettings):
    ...
    bootstrap_servers: str = Field(
        default=..., description='Kafka bootstrap servers', validation_alias='BROKER_BOOTSTRAP_SERVERS'
    )
    request_timeout_ms: int = Field(
        default=40 * 1000, description='Kafka request timeout ms', validation_alias='BROKER_REQUEST_TIMEOUT_MS'
    )
...
class BrokerKafkaConsumerSettings(BrokerKafkaSettings):
    ...
    topics: str = Field(default=..., description='Kafka topics', validation_alias='BROKER_TOPICS')
    group_id: str = Field(default=..., description='Kafka group id', validation_alias='BROKER_GROUP_ID')
    enable_auto_commit: bool = Field(
        default=False, description='Kafka enable auto commit', validation_alias='BROKER_ENABLE_AUTO_COMMIT'
    )
    max_poll_records: int = Field(
        default=100, ge=1, le=500, description='Kafka max poll records', validation_alias='BROKER_MAX_POLL_RECORDS'
    )
    ...
    heartbeat_interval_ms: int = Field(
        default=(15 * 1000), description='Kafka heartbeat interval ms', validation_alias='BROKER_HEARTBEAT_INTERVAL_MS'
    )
    session_timeout_ms: int = Field(
        default=(90 * 1000), description='Kafka session timeout ms', validation_alias='BROKER_SESSION_TIMEOUT_MS'
    )
    ...
    retry_max_times: int = Field(
        default=0, ge=0, le=3, description='Kafka retry max times', validation_alias='BROKER_RETRY_MAX_TIMES'
    )
    ...
    def _generate_retry_topics(self) -> list[str]:
        """Generate retry topics."""
        return [
            f'{topic}{BROKER_RETRY_SUFFIX}{i}'
            for topic in self._parse_topics(self.topics)
            for i in range(1, self.retry_max_times + 1)
        ]

    def _generate_dead_letter_queue_topics(self) -> list[str]:
        """Generate dead letter queue topics."""
        return [f'{topic}{BROKER_DEAD_LETTER_QUEUE_SUFFIX}' for topic in self._parse_topics(self.topics)]

    def get_topics(self) -> list[str]:
        """Create a list of topics with retry and dead letter queue topics."""
        topics = self._parse_topics(self.topics)
        topics.extend(self._generate_dead_letter_queue_topics())
        if self.retry_max_times > 0:
            topics.extend(self._generate_retry_topics())
        return topics
    ...
    def validate_topics_names(cls, topics: str) -> str:
        ...
        for topic in cls._parse_topics(topics):
            if not re.match(BROKER_TOPIC_PATTERN, topic):
                raise ValueError(f"Topic '{topic}' must follow the regex pattern: {BROKER_TOPIC_PATTERN}")
        return topics
    ...
    def validate_session_pool_timeouts(self) -> Self:
        ...
        if self.max_poll_interval_ms < self.session_timeout_ms:
            raise ValueError(
                f'\n    Kafka session max poll interval must be greater than session timeout.\n'
    ...
    def validate_session_heartbeat(self) -> Self:
        ...
        if self.session_timeout_ms / self.heartbeat_interval_ms < BROKER_HEARTBEAT_PER_SESSION:
            raise ValueError(
                f'\n    Kafka heartbeat per session must be greater than or equal to {BROKER_HEARTBEAT_PER_SESSION}.\n'
                f'      factor: {self.session_timeout_ms / self.heartbeat_interval_ms}\n'
                f'      session_timeout_ms: {self.session_timeout_ms}\n'
                f'      heartbeat_interval_ms: {self.heartbeat_interval_ms}\n'
            )
        return self
...
class BrokerKafkaProducerSettings(BrokerKafkaSettings):
    ...
    acks: BrokerKafkaAcks = Field(default=BrokerKafkaAcks.ALL, description='Kafka acks', validation_alias='BROKER_ACKS')
    connections_max_idle_ms: int = Field(
        default=10000, description='Kafka connections max idle ms', validation_alias='BROKER_CONNECTIONS_MAX_IDLE_MS'
    )
