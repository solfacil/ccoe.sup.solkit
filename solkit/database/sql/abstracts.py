...
class DatabaseSQLAdapterAbstract(ABC):
    ...
    @abstractmethod
    async def get_connection(self, session_type: DatabaseSQLSession = DatabaseSQLSession.READ_WRITE) -> AsyncGenerator:
        """Get a connection from the database."""
        raise NotImplementedError()

    @abstractmethod
    async def get_session(self, session_type: DatabaseSQLSession = DatabaseSQLSession.READ_WRITE) -> AsyncGenerator:
