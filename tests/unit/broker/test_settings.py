...
def test_create_common_settings_with_valid_environment_variables() -> None:
    ...
    environment_variables = {
        ...
        'BROKER_REQUEST_TIMEOUT_MS': '10000'
    }
...
def test_consumer_settings_get_topics_multiple_topics_with_retry() -> None:
    ...
    # assert
    expected_topics = [
        'SOME-TOPIC', 'ANOTHER-TOPIC',  # original topics
        'SOME-TOPIC-DLQ', 'ANOTHER-TOPIC-DLQ',  # dead letter queue topics
        'SOME-TOPIC-RETRY-1', 'SOME-TOPIC-RETRY-2',  # retry topics for topic 1
        'ANOTHER-TOPIC-RETRY-1', 'ANOTHER-TOPIC-RETRY-2'   # retry topics for topic 2
    ]
    assert topics == expected_topics
...
@pytest.mark.parametrize('acks_value,expected_result', [
